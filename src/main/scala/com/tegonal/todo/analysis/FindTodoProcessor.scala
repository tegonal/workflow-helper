...
class FindTodoProcessor(
  ...
  private val regex = Regex(s"((?:${todoIndicator.r.regex})\\s*(?:${issueIndicator.r.regex}))(?:\\s(.*))?")
  private val buffer = ListBuffer[Problem.Todos]()

  override def transformAndFilter(lineNumber: Int, line: String): Option[Todo] =
    regex.findFirstMatchIn(line).map(m => Todo(lineNumber, m.group(1), Option(m.group(2)).getOrElse("")))
